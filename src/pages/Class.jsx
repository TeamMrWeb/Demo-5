...
export default function Class() {
	...
	if (!checkClassName(name))
		return <NotFound description={'La clase que buscas no existe.'} />
		...
		<section className='class'>
			<div className='container'>
				<div className='class-header'>
					...
					<Button text='Volver a Actividades' to={`/actividades`} />
				</div>
				<div className='content'>
					<div className='info'>
						<p className='info__description'>
							{getObjectClass(name).description}
						</p>
						<ul className='features'>
							{getObjectClass(name).features.map(
								(feature, id) => {
									return (
					...
					<div className='image-container'>
						<img
							...
							src={getObjectClass(name).image}
							alt={`clase de ${name} en gimnasio de belgrano`}
						/>
					</div>
				</div>
